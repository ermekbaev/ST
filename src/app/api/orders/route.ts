...
async function saveOrderToStrapi(
  ...
  for (let index = 0; index < items.length; index++) {
    ...
    try {
      ...
      console.log(`📦 Товар: ${item.productName}`);
      ...
      const itemData = {
        ...
        productImage: imageUrl, // 🆕 ДОБАВИЛИ
        size: item.size, // 🆕 Сохраняем размер как текст
      ...
      if (!itemResponse.ok) {
        ...
        const fallbackData = {
          ...
          productId: item.productId,
          productName: item.productName || `Товар ${item.productId}`,
          productImage: imageUrl, // ✅ ДОБАВИТЬ
          size: item.size, // ✅ ДОБАВИТЬ
          quantity: item.quantity,
          priceAtTime: item.priceAtTime,
        };
